...
enum operationIds {START=0, THREAD_INIT, THREAD_EXIT, ENABLE_LIFECYCLE, TRIGGER_LIFECYCLE, ATTACH_Q,
	...
	WRITE_STATIC, ACCESS, ENABLE_EVENT, TRIGGER_EVENT, LOCK, FORK, JOIN, UNLOCK, WAIT, NOTIFY, INSTANCE_INTENT, TRIGGER_WINDOW_FOCUS, TRIGGER_BROADCAST, TRIGGER_SERVICE};

static const char * enumOpName[] = {"START", "THREADINIT", "THREADEXIT", "ENABLE_LIFECYCLE",
		"TRIGGER_LIFECYCLE", "ATTACH-Q", "NATIVE_ENTRY", "NATIVE_EXIT",
		"LOOP", "POST", "NATIVE-POST", "UI-POST", "DELAY-POST", "BEGIN", "END", "READ", "READ_STATIC", "WRITE",
		"WRITE_STATIC", "ACCESS", "ENABLE_EVENT", "TRIGGER-EVENT", "LOCK", "FORK", "JOIN",
		"UNLOCK", "WAIT", "NOTIFY", "INSTANCE_INTENT", "TRIGGER-WINDOW-FOCUS", "TRIGGER-BROADCAST", "TRIGGER-SERVICE"};
...
public:
	...
	virtual string writeToLogWithFormatting(){
		...
		for(int i=1; i<threadId; i++)
				sstm << "\t";
		if(opId == THREAD_INIT || opId == THREAD_EXIT)
